        ...
        children: [ 
          Row(
            children: [
              ...
              SizedBox(width: 20),
              Visibility(visible: (appState.selectedStudent != null) , child: Container( child: Row(children: [
                Text('Agregar Pago / Editar Alumno', style: Theme.of(context).textTheme.bodyLarge),
              IconButton(
            ...
            Text('Total de alumnos: ${snapshot.data!.length}', style: Theme.of(context).textTheme.bodyMedium),
             ...
             IconButton(
                ...
                onPressed: () {
                  setState(() {
                    appState.clearSelectedStudent();
                    _func = EventService().getStudentsByEvent(appState.appToken, appState.selectedEvent!.id);
                  });
                },
              ),
          ],),
        ...
        Expanded(
          ...
          rows: snapshot.data!.map((e) => PlutoRow( 
                  cells: {
                    ...
                    'paid': PlutoCell(value: e.payments.length > 0 ? e.payments.map((e) => e.amount).reduce((value, element) => value + element) :0),
                    'remaining': PlutoCell(value: e.totalCost - (e.payments.length > 0 ? e.payments.map((e) => e.amount).reduce((value, element) => value + element): 0)),
                    'student_status': PlutoCell(value: e.totalCost - (e.payments.length > 0 ? e.payments.map((e) => e.amount).reduce((value, element) => value + element): 0 ) == 0 ? 'Pagado' : 'Pendiente de pago'),
