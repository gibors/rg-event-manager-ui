...
class _EventPaymentPageState extends State<EventPaymentPage> {
  ...
  addPayment() {
  if(isGraduation){
    var paymentAmount = double.parse(_paymentAmount.text.isEmpty ? "0" : _paymentAmount.text);

    switch(_paymentDetail.text){
      case 'adicional':
        paymentAmount = selectedStudent!.pricing.additionalCost;
      case 'pre-fiesta':
      paymentAmount = selectedStudent!.pricing.additionalCost;
    ...
    _additionalNumber.clear();
     ...
     if (paid && selectedStudent.folio.isEmpty) {
      EventService().savePaymentAndFolioStudentData(payment, selectedStudent, token).then(
        (value) => {
          if(value != null && value.folio.isNotEmpty){
            setState(() {
                        ...
                        child: Column(children: [
                          Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                    'Total pagado: \$${paymentsHistory.isNotEmpty ? paymentsHistory.map((e) => e.amount).reduce((a, b) => a + b) : 0}',
                                    style: TextStyle(
                              ...
                              Expanded(
                                child: Text(
                                    'Total a pagar: \$${selectedStudent != null ? selectedStudent.totalCost : 0}',
                                    style: TextStyle(
                                        fontSize: 14.0, color: Colors.black)),
                              ),
                              SizedBox(width: 20),
