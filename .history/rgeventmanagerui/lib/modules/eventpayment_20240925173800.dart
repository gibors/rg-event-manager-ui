...
@override
  void initState() {
    ...
    if(selectedEvent != null && selectedEvent!.eventType.id == 3) {
      ...
      isGraduation = false;
       EventService().getPaymentsByEventId(token, selectedEvent!.id).then((payments) {
        setState(() {
  ...
  addPayment(){
    ...
    if(_formKey.currentState!.validate()) {
      ...
      EventService().createPayment(payment, token).then((value) {
        success = true;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Pago agregado')));
      }, onError: (error) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error al agregar el pago')));
      });
      setState(() {
        if(success) {
          ...
          child: Column(
            ...
            Row(children: [
                Expanded(child: TextFormField(
                  ...
                  validator: (value) => value != null && value!.isEmpty ? 'El monto del pago es requerido' : null,
                )),
                SizedBox(width: 20),
                Expanded(child: RawAutocomplete<String>(
                ...
                fieldViewBuilder: (BuildContext context, TextEditingController textEditingController, FocusNode focusNode, VoidCallback onFieldSubmitted) {
                  return TextFormField(
                    ...
                    validator: (value) => value != null && value.isEmpty ? 'El método de pago es requerido' : null,
                  );
                }, 
                optionsViewBuilder: (BuildContext context, void Function(String) onSelected, Iterable<String> options) { 
                  return Material(
              ...
              Row( 
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                   if((selectedStudent != null || !isGraduation ) && _formKey.currentState!.validate()) {
                     var payment = Payment(
                      id: -1,
                       amount: double.parse(_paymentAmount.text),
                       paymentMethod: _paymentMethodController.text,
