...
class _EventPaymentPageState extends State<EventPaymentPage> {
  ...
  addPayment() {
    ...
    var payments = paymentsHistory
        ...
        .toList();
    var totalPaid = payments.isNotEmpty ? payments.map((e) => e.amount).reduce((a, b) => a + b) + double.parse(_paymentAmount.text) : 0;
    var format = NumberFormat('###.00', 'es_MX');
    if( totalPaid > selectedStudent!.totalCost){
      Flushbar(
        flushbarPosition: FlushbarPosition.TOP,
        title: 'Error',
        message: 'El pago excede el costo total del paquete/platillo, solo puedes cobrar  ${NumberFormat.currency(locale: 'mx').format(totalPaid - selectedStudent!.totalCost)} y despues agregar pagos adicionales u otro servicio',
