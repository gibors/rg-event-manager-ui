...
//shared-preferences
  ...
  Future<Student> savePaymentAndStudentData(Payment payment, Student student , String token) async{
    try{
      ...
      log('folioData: $folioData');
      if (folioData != null){
        student.folio = folioData.toString();
      
      
      var requests = [_dio.post('http://localhost:8080/api/v1/payments', data: payment.toJson(),
            options: Options(headers: {
          'Authorization': 'Bearer $token'})), 
      _dio.post('http://localhost:8080/api/v1/students', data: student.toJson(),
            options: Options(headers: {
          'Authorization': 'Bearer $token'}))];
          
      var responses = await Future.wait(requests);
      final paymentResponse = Payment.fromJson(responses[0].data);
