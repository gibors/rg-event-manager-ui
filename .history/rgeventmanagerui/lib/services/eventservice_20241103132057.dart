...
//shared-preferences
  ...
  Future<List<Payment>> getPaymentsByEventId(String token, int eventId) async {
    try {
      final response = await _dio.get('http://localhost:8080/api/v1/payments/events/$eventId',
      options: Options(headers: {
          'Authorization': 'Bearer $token'}));
      final data = response.data as List<dynamic>;
  ...
  Future<Student> savePaymentAndFolioStudentData(Payment payment, Student student , String token) async{
    try{
      Student? studentResponse;
      var folioRequest = await _dio.get('http://localhost:8080/api/v1/students/folio',
      ...
      var requestPayment = _dio.post('http://localhost:8080/api/v1/payments', data: payment.toJson(),
            options: Options(headers: {
          'Authorization': 'Bearer $token'}));

      var requestStudent = _dio.post('http://localhost:8080/api/v1/students', data: student.toJson(),
            options: Options(headers: {
          'Authorization': 'Bearer $token'}));
          
      var responses = await Future.wait([requestStudent]);

      final studentReponse = Payment.fromJson(responses[0].data);
      
       studentResponse = Student.fromJson(responses[1].data);
      }
      return studentResponse!;
  ...
  Future <List<Supplier>> getAllProviders(String token) async {
      try {
      final response = await _dio.get('http://localhost:8080/api/v1/suppliers',options: Options(headers: {
          'Authorization': 'Bearer $token'}));
      final data = response.data as List<dynamic>;
      final providers = data.map((provider) => Supplier.fromJson(provider)).toList();
      return providers;
    } catch (e) {
      throw Exception('Failed to fetch event types: $e');
    }
  ...
  Future<List<ServiceType>> getServices(String token) async {
    try {
      final response = await _dio.get('http://localhost:8080/api/v1/suppliers/service-types',options: Options(headers: {
          'Authorization': 'Bearer $token'}));
      final data = response.data as List<dynamic>;
      final services = data.map((service) => ServiceType.fromJson(service)).toList();
      return services;
    } catch (e) {
