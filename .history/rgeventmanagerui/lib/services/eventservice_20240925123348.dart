...
//shared-preferences
  ...
  Future <List<Student>> getStudentsByEvent(String token, int eventId) async {
    ...
    Future <List<Student>> saveStudent(String token, int eventId) async {
    try {
      final response = await _dio.post('http://localhost:8080/api/v1/students',options: Options(headers: {
          'Authorization': 'Bearer $token'}));
