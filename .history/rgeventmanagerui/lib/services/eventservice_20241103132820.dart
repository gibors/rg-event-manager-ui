...
//shared-preferences
  ...
  Future<Student> savePaymentAndFolioStudentData(Payment payment, Student student , String token) async{
    try{
      ...
      var requestPayment = await _dio.post('http://localhost:8080/api/v1/payments', data: payment.toJson(),
            options: Options(headers: {
          'Authorization': 'Bearer $token'}));
      ...
      var requestStudent = await _dio.post('http://localhost:8080/api/v1/students', data: student.toJson(),
            options: Options(headers: {
          'Authorization': 'Bearer $token'}));
       studentResponse = requestStudent.data;
      }
      return studentResponse!;
